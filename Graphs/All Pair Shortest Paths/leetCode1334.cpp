...
int32_t main() {
    ...
    vector<vector<int>> next(N + 1, vector<int>(N + 1, -1)); // INDEX
    ...
    // path tracing
    int best_city = -1; // which row
    int best_city_destinations = intmax; // minimized
    ...
    for (int city = 1; city <= n; city++) {
        ...
        for (int dest = 1; dest <= n; dest++) {
            ...
            if (dp[city][dest] <= threashold) {
                destinations++;
            }
        }

        if (best_city_destinations <= destinations) { // '<=' so that if tie, I get the highest indexed node as required
            best_city_destinations = destinations;
            best_city = city;
        }
    }
