...
int32_t main() {
    ...
    while (true) {
        ...
        if (dp[start][w1] == intmax || dp[w2][end] == intmax) {
            cout << "No path from " << start << " to " << end << " through Wall Street : {" << w1 << ", " << "}"; 
            return 0;
        }

        cout << "Shortest Path Weight: " << dp[start][end];
        cout << "Path: ";

        int current = start;
        cout << current << " -> ";
        while (current != w1) {
            current = dp[current][end];
            cout << current << " -> ";
        }
        current = w2;
        cout << w2 << " -> ";
